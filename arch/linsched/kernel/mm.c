...
{
	return kzalloc(cachep->size, flags);
}

__attribute__ ((weak)) void *linsched_malloc(unsigned long size)
{
	return NULL;
}
...
void __init kmalloc_init(void)
{
	...
	for (i=0;i<sizeof(malloc_sizes)/sizeof(malloc_sizes[0]);i++) {
		struct kmem_cache *cache = linsched_malloc(sizeof(struct kmem_cache));

		cache->size = malloc_sizes[i].cs_size;
...
__attribute__ ((weak)) void *__kmalloc(size_t size, gfp_t flags)
{
	return NULL;
}
...
__attribute__ ((weak)) void kfree(const void *block)
{
}
...
/*
 * We don't really care about this one because we don't handle
 * any sort of memory management
 */
