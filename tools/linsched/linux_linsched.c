...
void linsched_init_root_cgroup(struct cgroup *root)
{
	...
	root_cgroup->subsys[cpuacct_subsys_id] = cpuacct_subsys.create(root_cgroup);
}
...
struct cgroup *linsched_create_cgroup(struct cgroup *parent, char *path)
{
	...
	child->parent = parent;
	child->subsys[cpu_cgroup_subsys_id] = cpu_cgroup_subsys.create(child);
	child->subsys[cpu_cgroup_subsys_id]->cgroup = child;
	child->subsys[cpuacct_subsys_id] = cpuacct_subsys.create(child);
...
void *linsched_malloc(unsigned long size)
{
	return malloc(size);
}
